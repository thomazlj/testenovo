...
function updateUI() {
  ...
  document.getElementById("totalDayTimer").textContent =
    formatHHMM(totalDaySeconds);
...
// NOVA FUNÇÃO
// ===============================
function resetDayTotal() {
  addHistory(`⏱️ Total do dia zerado — ${formatHHMM(totalDaySeconds)}`);
  totalDaySeconds = 0;
  updateUI();
}
...
setInterval(() => {
  ...
  for (let i = 0; i < speed; i++) {
    ...
    if (state === "study") {
      studyTime--;
      totalDaySeconds++; // ⏱️ conta foco real
